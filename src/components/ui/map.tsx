...
/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect -- vendored
   from mapcn (github.com/AnmolSaini16/mapcn); its "latest ref", one-time
   useMemo-init, and staged style-swap effect patterns are deliberate and safe
   (see inline comments below) but predate this project's stricter React
   Compiler lint rules. Keep in sync with upstream rather than reworking to
   satisfy these rules. */
  ...
  return (
    <div className="bg-background/50 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
      <div className="flex gap-1">
        <span className="bg-herbal-muted/60 size-1.5 animate-pulse rounded-full" />
        <span className="bg-herbal-muted/60 size-1.5 animate-pulse rounded-full [animation-delay:150ms]" />
        <span className="bg-herbal-muted/60 size-1.5 animate-pulse rounded-full [animation-delay:300ms]" />
      </div>
    </div>
  );
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="hover:bg-herbal-soft text-herbal-ink absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-herbal-brown"
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-white text-herbal-ink relative max-w-62 rounded-md border p-3 shadow-md",
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-herbal-green/15 bg-background [&>button:not(:last-child)]:border-herbal-green/15 flex flex-col overflow-hidden rounded-md border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
  ...
  return (
    <ControlButton onClick={onClick} label="Reset bearing to north">
      ...
      >
        ...
        <path d="M12 22L16 12H12V22Z" className="fill-herbal-muted/60" />
        <path d="M12 22L8 12H12V22Z" className="fill-herbal-muted/30" />
      </svg>
    </ControlButton>
  );
}
